...
void update_d(nj_data_t data, otu_t pos, otu_t *pairs);
...
void update_d(nj_data_t data, otu_t pos, otu_t *pairs)
{
    ...
    {
        int cont = 0;
        for (int pair = 0; pair < data.N * data.p; pair++)
    ...
    for (size_t pair = 0; pair < data.N * data.p; pair++)
    {
        int pair_i = pairs[pair].i;
        int pair_j = pairs[pair].i;

        float d_ac, d_ad, d_bc, d_bd;
        float d_cd = d_get_D_position(data, pair_i, pair_j);
        ...
        new_duk = (d_ac + d_ad - 2 * d_ij + d_bc + d_bd - 2 * d_cd) / 4;

        data.S[pair_i] = data.S[pair_i] + new_duk - (new_duk * 2 + d_ij);
